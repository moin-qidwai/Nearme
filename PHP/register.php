<?php
	...
	else
	{
		...
		$sql = "INSERT INTO user_info (name,number,country,display_name,gcm_regid) VALUES (:name,:number,:country,:display_name,:gcm_regid)";
		...
		$rows = $sth->fetchAll();
		echo '{ status : 1, "ID" : '.$rows[0]['ID'].', "number" : "'.$rows[0]['number'].'", "name" : "'.$rows[0]['name'].'", "countryCode" : "'.$rows[0]['country'].'", "displayName" : "'.$rows[0]['display_name'].'", "gcmId" : "'.$rows[0]['gcm_regid'].'" }';
