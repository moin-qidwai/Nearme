<?php
	...
	try {
		$ssth = $dbh->prepare('SELECT * FROM user_info WHERE ID = '.$sourceId);
		$ssth->execute();
	}
	catch(Exception $e)
	{
	...
	$sourceRows = $ssth->fetchAll();
	if(sizeof($sourceRows))
	{
		$sourceLat = $sourceRows[0]['lat'];
		$sourceLon = $sourceRows[0]['lon'];
	}
	...
	echo '{ "status" : 1 ,"sourceId" : '.$sourceId.' , "lons" : "'.implode(",", $lons).'", "lats" : "'.implode(",", $lats).'", "targetIds" : "'.implode(",", $rTargetIds).'", "sourceLat" : '.$sourceLat.', "sourceLon" : '.$sourceLon.'}';
